...
ISR(PCINT1_vect) {
  ...
  for (uint8_t b = 0; b <= 5; b++) {
    uint8_t idx = b;               // ch 0..5
    if (ch[idx].armed && (changed & (1 << b))) {
      uint8_t level = (now >> b) & 1;
      uint16_t t = TCNT1;
      if (level && ch[idx].stage == 0) {
        uint32_t since = (g_ovf - ch[idx].ovf_fire)*65536UL + (uint32_t)t - ch[idx].t_fire;
        if (since < ch[idx].blank_us) return;  // abaikan rising terlalu dini
        ch[idx].t_start = t;
        ch[idx].ovf_start = g_ovf;
        ch[idx].stage = 1;         // tunggu falling
      } else if (!level && ch[idx].stage == 1) {
        uint32_t ticks = (g_ovf - ch[idx].ovf_start) * 65536UL + (uint32_t)t - ch[idx].t_start;
        ch[idx].tof_us = ticks;
        ch[idx].ready = 1;
        ch[idx].stage = 0;
        ch[idx].armed = 0;
...
void pcint_begin() {
  ...
  PCMSK1 |= (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5); // PC0(bit0), PC1(bit1) - PC5
}
...
void setup() {
  ...
  for (uint8_t i=0; i<8; i++) pinMode(ECHO[i], INPUT);
